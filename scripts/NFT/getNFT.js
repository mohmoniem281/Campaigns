...
async function main() {
  ...
  const NFT = await ethers.getContractFactory("PrizeToken");
  const nft_contract = await NFT.attach(process.env.NFT_TOKEN_ADDRESS);
  console.log("Token "+await nft_contract.connect(deployer_wallet).balanceOf(await fourth_user_wallet.getAddress()));
